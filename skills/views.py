...
def profile_view(request):
    ...
    conversations = (
        ...
        .select_related("listing", "listing__skill")
...
def edit_profile(request):
    ...
    return render(request, "skills/edit_profile.html", {"form": form})
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if not geo:
                form.add_error(
                    "location_text",
                    "Could not find that location. Try a postcode or full town/city name.",
                )
            else:
                ...
                messages.success(
                    request,
                    f"Success! Your listing '{listing.skill.name}' has been published.",
                )
...
def search(request):
    ...
    listings = (
        Listing.objects.select_related(
            "skill", "provider", "provider__user", "location"
        )
    ...
    form = ListingForm(
        request.GET, search_mode=True
    )  # Pass search_mode=True to the form to adjust its behavior
...
def edit_listing(request, listing_id):
    ...
    initial = {
        "location_text": listing.location.name if listing.location else ""
    }
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                f'Changes to "{updated.skill.name}" saved successfully!',
            )
...
def listing_detail(request, listing_id):
    ...
    rating_stats = listing.reviews.aggregate(
        avg=Avg("rating"), count=Count("id")
    )
    reviews = listing.reviews.select_related(
    ...
    if request.user.is_authenticated:
        ...
        if reviewer != listing.provider:
            ...
            has_replied = (
                listing.conversations.filter(participants=reviewer)
                .filter(messages__sender=listing.provider)
                .exists()
            )
            if has_replied:
                can_review = True
                ...
                user_review = Review.objects.filter(
                    listing=listing, reviewer=reviewer
                ).first()
            ...
            if request.method == "POST":
                ...
                if review_form.is_valid():
                    ...
                    msg = (
                        ...
                        if user_review
                        else "Your review has been submitted."
                    )
                    messages.success(request, msg)
                    return redirect(
                        f"{reverse('listing_detail', args=[listing.id])}#review-section"
                    )
            else:
                review_form = ReviewForm(instance=user_review)
    return render(
        request,
        "skills/listing_detail.html",
        {
            "listing": listing,
...
def delete_review(request, review_id):
    ...
    review = get_object_or_404(
        Review, id=review_id, reviewer=request.user.profile
    )
    ...
    return redirect(
        f"{reverse('listing_detail', args=[listing_id])}#review-section"
    )
...
def start_conversation(request, listing_id):
    ...
    existing = (
        listing.conversations.filter(participants=me)
...
def conversation_detail(request, conversation_id):
    ...
    return render(
        request,
        "skills/conversation_detail.html",
        {
            "conversation": conv,
...
def delete_profile(request):
    ...
    user.delete()
    messages.success(
        request, "Your account and all associated data have been deleted."
    )
    return redirect("home")
